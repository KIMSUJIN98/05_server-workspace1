...
public class RequestPostServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// POST 방식 요청 같은 경우는
		// 뽑기 전!!! *********에 인코딩 설정 해야됨 => UTF-8 로!!
		request.setCharacterEncoding("UTF-8");
		
		String name = request.getParameter("name"); // "차은우" | ""
		String gender = request.getParameter("gender"); // "M" | "F" | null
		int age = Integer.parseInt(request.getParameter("age")); // "20" => 20 | "" => 오류가뜸 (NumberformatException) : 빈문자열 숫자로 변환 불가
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		doGet(request, response); // POST 방식이지만 어짜피 doGet(GET 방식)을 호출하므로 doGet에 출력하고자 하는 내용을 작성해도 된다.
	}
